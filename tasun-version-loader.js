(function(){
  ...
  function parseConfig(raw){
    ...
    var fallback = norm(raw.fallbackVersion || raw.manualVersion || raw.appVer || raw.APP_VER || raw.ver || raw.version || raw.appVersion || "") || "20260319_16";
  ...
  (async function(){
    try{
      ...
      if(!ver) ver = cfg.fallbackVersion || initial || "20260319_16";
      setGlobals(ver);
      saveCache(ver, { mode: cfg.mode, current: currentBaseName() });
      maybeRedirect(ver);
      READY_RESOLVE(true);
    }catch(e){
      var fallback = initial || "20260319_16";
      setGlobals(fallback);
      READY_RESOLVE(true);
    }
  })();
