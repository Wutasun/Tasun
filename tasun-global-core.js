(function(global){
  'use strict';
  var CORE_VER = '20260320_03_global_core_v2';
  ...
  function getCurrentUser(){
    var keys = [CURRENT_KEY, SESSION_KEY, 'tasunSessionBridge_v1', 'tasunSso_v2', 'tasunSso_v1'];
    for(var i=0;i<keys.length;i++){
      var row = safeJSON(readSession(keys[i]) || readLocal(keys[i]));
      if(row && (row.user || row.name || row.username)){
        return { user:norm(row.user || row.username || row.name), name:norm(row.name || row.user || row.username), role:normalizeRole(row.role || row.level), at:norm(row.at || nowISO()) };
      }
    }
    return null;
  }
  ...
  function setCurrentUser(user){
    ...
    var row = JSON.stringify({ user:norm(user.user || user.name), name:norm(user.name || user.user), role:normalizeRole(user.role || 'read'), at:nowISO() });
  ...
  function clearSessionLogin(){
    [CURRENT_KEY, SESSION_KEY, INDEX_SESSION_KEY, 'tasunSessionBridge_v1', 'tasunSso_v2', 'tasunSso_v1'].forEach(function(k){ try{ sessionStorage.removeItem(k); }catch(_e){} });
  }
  ...
  function normalizeAuthTable(){
    var t = safeJSON(readLocal(AUTH_KEY)) || {};
    var users = Array.isArray(t.users) ? t.users : Array.isArray(t.rows) ? t.rows : Array.isArray(t) ? t : [];
    return users.map(function(u){ return { user:norm(u.user || u.username || u.name), name:norm(u.name || u.user || u.username), role:normalizeRole(u.role || 'read'), pass:norm(u.pass || u.password || u.pwd || u.secret || u.passwd || '') }; }).filter(function(u){ return !!u.user; });
  }
  function getNamedButtons(){
    var raw = safeJSON(readLocal(BUTTONS_KEY)) || {};
    var list = Array.isArray(raw.buttons) ? raw.buttons : Array.isArray(raw) ? raw : [];
    return list.map(function(x, idx){ return { key:norm(x.key || ('btn' + (idx+1))), name:norm(x.name || x.label || x.title || ('btn' + (idx+1))), href:norm(x.href || x.url || ''), target:norm(x.target || '') }; });
  }
  ...
  function getRouteMap(){
    ...
    [ROUTES_KEY, 'tasunRoutes_v1'].forEach(function(key){ var stored = safeJSON(readLocal(key)); if(stored && typeof stored === 'object'){ Object.keys(stored).forEach(function(k){ var nk=norm(k), nv=norm(stored[k]); if(nk&&nv) map[nk]=nv; }); } });
    getNamedButtons().forEach(function(btn){ if(btn.name && btn.href) map[btn.name] = btn.href; });
    return map;
  }
  ...
  function getCloudToken(){ var token = readSession(CLOUD_TOKEN_KEY) || readSession('tasunBearerToken_v1') || readLocal(CLOUD_TOKEN_KEY) || readLocal('tasunBearerToken_v1') || ''; return norm(token).replace(/^Bearer\s+/i,''); }
  ...
  global.TasunGlobalCore = { version:CORE_VER, nowISO:nowISO, addV:addV, getCurrentUser:getCurrentUser, setCurrentUser:setCurrentUser, clearSessionLogin:clearSessionLogin, requireLoginOnColdStart:requireLoginOnColdStart, normalizeAuthTable:normalizeAuthTable, getNamedButtons:getNamedButtons, getRouteMap:getRouteMap, resolveRouteByName:resolveRouteByName, navigateByName:navigateByName, applyButtonRoutes:applyButtonRoutes, getCloudToken:getCloudToken, buildAuthHeaders:buildAuthHeaders, fetchJson:fetchJson, syncBootstrap:syncBootstrap };
})(window);
