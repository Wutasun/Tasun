...
(function(global){
  ...
  function mirrorToken(token){
    ...
    TOKEN_KEYS.forEach(function(k){
      if(token){ write(sessionStorage,k,token); write(localStorage,k,token); }
      else { removeBoth(k); }
    });
    return token;
  }
  ...
  function bridgeCurrentToChild(nextPath){
    ...
    try{
      ...
      localStorage.setItem(INDEX_SESSION_KEY,'1');
      if(nextPath){
        sessionStorage.setItem(NEXT_KEY, String(nextPath));
        localStorage.setItem(NEXT_KEY, String(nextPath));
      }
    }catch(_e){}
    return true;
  }
  ...
  async function tryWorkerLogin(apiBase, username, password){
    ...
    var token=norm(
      data.token||data.accessToken||data.authToken||data.bearer||
      bag.token||bag.accessToken||bag.authToken||bag.bearer
    );
  ...
  async function login(opts){
    ...
    var u=await tryWorkerLogin(apiBase,username,password);
    setSessionState(u,password);
    return getCurrent();
  }
